...
export default function Netflix() {
  ...
  useEffect(() => {
    ...
    return;
      ...
      const animate = () => {
        ...
        if (elapsed < 6500) { // Animation duration
    ...
    const intervalId = setInterval(animateLetters, 7000); // Change colors every 7 seconds
  ...
  return (
    <div className="relative w-full h-screen bg-black text-white overflow-hidden">
      {/* Scrolling Image Collage with Dimming Overlay */}
      <motion.div
        className="absolute top-0 left-0 w-full h-full flex"
        animate={{ x: ['0%', '-100%'] }}
        transition={{ repeat: Infinity, duration: 175, ease: 'linear' }}
      >
        <div className="flex">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="relative h-screen w-screen">
              <img
                src="https://pub-1c024d6181db4a8d9ce127046cdbc59f.r2.dev/Backgroundimg.png"
                alt=""
                className="h-screen w-screen object-cover brightness-50 filter"
              />
            </div>
          ))}
        </div>
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-black/70 via-black/50 to-black/80 z-5"></div>
      
      {/* Overlay Content with Premium Styling */}
...
>
  ...
  >
    ...
    <circle cx="100" cy="100" r="5" fillOpacity="0.9" />
            ...
            <h1 
              ref={subHeadlineRef}
              className="text-5xl font-black tracking-wide drop-shadow-xl py-0"
            >
              Generate It!
            </h1>
          ...
          <motion.button
            ref={buttonRef}
            className="px-10 py-5 text-lg font-semibold text-neutral-900 rounded-full hover:opacity-90 transition-all duration-300 transform shadow-lg hover:shadow-xl"
      ...
      {/* Wave Section with Smaller Crest */}
